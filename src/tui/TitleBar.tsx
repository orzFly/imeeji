...
export function TitleBar({ title, subtitle }: TitleBarProps) {
  ...
  const leftPad = Math.max(
    0,
    Math.floor(columns / 2) - Math.floor(centerLen / 2) - leftLen,
  );
  const rightSuffix = rightPart ? rightPart + " " : "";
  const rightPad = Math.max(
    0,
    columns - leftLen - leftPad - centerLen - rightSuffix.length,
  );

  const line = leftPart + " ".repeat(leftPad) + centerPart +
    " ".repeat(rightPad) + rightSuffix;
  const paddedLine = line.length >= columns
